...
class ModuleThread : public MsgThread
{
public:
    ModuleThread(const char * threadName = NULL);
    virtual ~ModuleThread() { };
    virtual void onDispatchMessage(UINT8* pData, int dataLen);
    virtual void onIdle();

    // Dispatch Message: m_msgID = MSG_ID_TEST
    void onTest(UINT8* pData, int len);

    // Dispatch Message: m_msgID = MSG_ID_ON_CHANNEL_OPENED
    void onChannelOpened(UINT8* pData, int len);

    // Dispatch Message: m_msgID = MSG_ID_ON_CHANNEL_CLOSED
    void onChannelClosed(UINT8* pData, int len);

    // Dispatch Message: m_msgID = MSG_ID_ON_CHANNEL_MESSAGE
    void onChannelMessageReceived(UINT8* pData, int len);
};
