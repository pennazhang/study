...
MsgThread::MsgThread(const char * threadName) : Thread(threadName)
{
}

void MsgThread::sendMessage(Serializable *obj)
{
    int dataSize;
    UINT8 *pData = obj->serializeToBuffer(dataSize);
    m_msgQueue.putMsg(pData, 0, dataSize, true);
}

void MsgThread::cleanAllMessage()
{
    m_msgQueue.clean();
}

void MsgThread::onIdle()
{
}
...
void *MsgThread::run()
{
    for ( ; ; ) 
    {
        ...
        int msgLen;
        UINT8 *pData = m_msgQueue.getMsg(msgLen);
        if (pData != NULL)
        {
            onDispatchMessage(pData, msgLen);
            delete pData;
        }
        else
        {
