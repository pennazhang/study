...
class CmsfsFuncCallbacks
{
public:
    typedef std::map<std::string, std::function<void(std::string)>> CmsfsFuncMap_t;
    void registerFunc(std::string keyName, std::function<void(std::string)> func)
    {
        m_funcMap[keyName] = func;
    };
    bool findFuncExec(std::string keyName, std::string keyValue)
    {
        for (const auto& i : m_funcMap)
        {
            if(keyName == i.first)
            {
                i.second(keyValue);
        ...
        // Can't find any matched key name.
        return false;
    };
private:
    CmsfsFuncMap_t m_funcMap;
};
