...
/***************************************************
 * Read hex array from a string.
For Example:  readHexString (aucMac, "23 12 9a ba 08 00", 6);
*****************************************************/
int readHexString(UINT8 *pucData, char *lpszBuffer, int nMaxLen)
{
    ...
    for (i = 0; i < nMaxLen; i++)
    {
        ...
        if ((isValidHex(cData[0]) == TRUE) && (isValidHex(cData[1]) == TRUE))
        {
...
void d(void *pAddr, int nByteCount, int nWidth, int nDisplayOffset)
{
	...
	nStart = (SYSWORD)pAddr;
	nEnd = (SYSWORD)pAddr + nByteCount;
...
VOS_STATUS ReadPhyMem(void *pDestAddr, void * pPhyMem, int nByteCount)
{
	...
	if (ulKernelAddr == 0)
	{
		printf("Memory Block is too big!\n");
		return (VOS_ERROR);
	}
...
VOS_STATUS CopyFileToMemory(UINT8 *pucByte, int nCount, char *lpszFileName, int nOffset)
{
    ...
    if (nTotal != nCount)
    {
        printf("Read file failed! Total Read = %d\n", nTotal);
        return (VOS_ERROR);
    }
    else
    {
...
VOS_STATUS cp(char *lpszOldFileName, char *lpszDstFileName, int nCopyUnit, int nStart, int nEnd)
{
	...
	if (pucByte == NULL)
	{
		printf("CopyUnit is too big to malloc memory!\n");
		goto cp_end;
	}
...
UINT16 crc(UINT8 *ptr,  int nLen) 
{
	...
	while(nLen--!=0) 
	{
		ucData1 = (UINT8)(crc/256);    /* high 8 bit  */
		crc = crc << 8;              
		ucData2 = *ptr;
		ucData3 = ucData2 ^ ucData1;
		crc = crc ^ crc_ta[ucData3];   
		ptr++;
	}
	return(crc);
}
...
unsigned long getTickCount()
{
    ...
    tick = (unsigned long)(tv.tv_sec * 1000 + tv.tv_usec / 1000);
    return (tick);
}
#if 0
...
//            printf("Last execute func burst for %d time at tick: %d\n", nBurstCount, ulCurrentTick);
...
STATUS runScript(char *fileName)
{
    ...
    while (fgets(line, 1024, script) != NULL)
    {
		...
		if (ExplainCmd(line, &result) == STATUS_OK)
		{
