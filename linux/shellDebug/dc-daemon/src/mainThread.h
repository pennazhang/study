...
class MainThread : public MsgThread
{
public:
    MainThread(const char * threadName = "MainThread");
    virtual ~MainThread();
...
//    void onSendChannelMessage(UINT8* pData, int len);
    ...
    // We must end all sub-thread in onClose function.
...
protected:
    ...
    UDPSocketThread* m_udpSocketThread;
    std::vector<TCPSocketThread *> m_tcpSocketThreadVector;
};
