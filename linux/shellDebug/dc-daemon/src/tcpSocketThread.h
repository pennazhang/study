...
{
public:
    TCPSocketThread(TCPSocket *tcpSocket);
    virtual ~TCPSocketThread();

    // It will be called when the thread is start.
    virtual STATUS onStart();

    // call this function from other task will triger the quit of this task.
    virtual void quit();
    virtual void *run();
