...
#define MAX_UDP_FRAME_SIZE 1518

/*---------------------------------------------------------------------------*
 * Class: UDPSocket
 *---------------------------------------------------------------------------*/
/**
 * A UDPSocket is merely a Socket that was created for IPv4 based
 * UDP communications.
 *
 * <!------------------------------------------------------------------------*/
...
public:
    UDPSocket();
    ~UDPSocket();
    STATUS bind(UINT16 port);
    STATUS bind(const char *localAddress, UINT16 localPort);
    void sendTo(sockaddr_in& peerAddr, UINT8 *pData, int byteslength);
    void sendTo(sockaddr_in& peerAddr, std::string message);
    SocketEventType checkEvent();
    UINT8 *getReceivedData(int& len);
    int getChannelID();
    sockaddr_in* getPeerIP() { return &m_peerAddr; };
    
private:
    std::string m_address;
    UINT16 m_localPort;

    /* only valid when checkEvent() == SOCKET_EVENT_RECV_DATA */
    UINT8 m_buffer[MAX_UDP_FRAME_SIZE];
    int m_efficientLen;
