...
//
// Just for SSL Server based on TCP Server
//
...
public:
    ...
    /* Listen at TCP Port with IP address = 0.0.0.0 */
	STATUS listenAt(UINT16 port);

	STATUS listenAt(const char *address, UINT16 port);
	
    /* We will set the socket to non-block mode, so checkIncomingConnection is a non-block function. */
    // It should be called periodically.
    SecureTCPSocket * checkIncomingConnection();
};
