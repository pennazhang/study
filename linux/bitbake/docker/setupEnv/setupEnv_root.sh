...
if [ ! -f ~/.bitbake_apt_done ]; then
    # development
    apt update || true
    apt-get install -y git build-essential cmake autogen autoconf libtool m4 pkg-config bc device-tree-compiler python3.8 wget cpio unzip rsync gnupg2 libc6-i386 lib32stdc++6 lib32z1 dos2unix iputils-ping net-tools iproute2
