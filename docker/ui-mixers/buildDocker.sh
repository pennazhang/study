...
CURRENT_DIR=`dirname "$0"`; CURRENT_DIR=`realpath "$CURRENT_DIR"`; export CURRENT_DIR

# The source code from $GIT_REPO must be put into $HOST_GIT_DIR/$PROJECT_NAME
HOST_GIT_DIR=/home/share/pzhang/git
PROJECT_NAME=ui-mixers
DOCKER_IMAGE_NAME=uimixer-docker
GIT_REPO=ssh://git@bitbucket-munich.harman.com:7999/bitbucket/hpromixer/ui-mixers.git
...
AUTO_START_DIR=/git/$PROJECT_NAME/scripts/ubuntu_install_dev
AUTO_START_SCRIPT=install_dev.sh
...
if [ -d "$CURRENT_DIR/${HOST_SRC_DIR}" ] && [ ! -z $DOCKER_DST_DIR ];then
    ...
    mkdir -p $HOST_GIT_DIR/$PROJECT_NAME/${DOCKER_DST_DIR}
...
imageExistFlag="$(docker images $DOCKER_IMAGE_NAME:0.1 | grep $DOCKER_IMAGE_NAME)"
...
TEMP_CONTAINER_NAME=temp_container

#Check if the image is already exist.
imageExistFlag="$(docker images $DOCKER_IMAGE_NAME:0.2 | grep $DOCKER_IMAGE_NAME)"
if [ -z "$imageExistFlag" ];then
    echo ------------------------------------------------------------------------------
